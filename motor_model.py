...
def induction_motor_simulation(voltage, frequency, poles, slip, load_torque, 
rheostat_resistance):
    """
...
def synchronous_motor_simulation(voltage, frequency, poles, load_torque, 
excitation_factor, rheostat_resistance):
    """
...
# ==================== NEW: DC MOTOR SIMULATION ====================

def dc_motor_simulation(voltage, armature_resistance, field_flux, load_torque, 
rheostat_resistance):
    """
    Simulates DC Motor (Separately Excited) behavior.
    """
    # Armature current
    base_current = (load_torque * 0.7 + voltage * 0.025)
    current = base_current / (1 + rheostat_resistance)

    # Back EMF
    ra_total = armature_resistance + rheostat_resistance
    back_emf = voltage - (current * ra_total)

    # Speed (RPM)
    speed = (back_emf * 60) / (field_flux * 2 * np.pi * 0.9)   # Simplified realistic formula

    # Developed Torque
    torque = (field_flux * current * 1.2) * (1 / (1 + rheostat_resistance * 0.4))

    # Power calculations (DC Motor)
    input_power = (voltage * current) / 1000  # kW
    output_power = (2 * np.pi * speed * torque) / 60000  # kW
    apparent_power = input_power
    reactive_power = 0.0

    # Losses
    armature_loss = (current ** 2) * armature_resistance / 1000
    total_loss = armature_loss + (input_power * 0.04)
    ...
    return {
        ...
        "Field Flux (Wb)": round(field_flux, 3),
        "Rheostat Resistance (Ohm)": round(rheostat_resistance, 2),
